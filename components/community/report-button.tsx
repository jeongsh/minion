...
export function ReportButton({ target, postId, commentId, scope, teamSlug }: { target: "post" | "comment"; postId: string; commentId?: string; scope: BoardScope; teamSlug?: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-2">
      <button type="button" onClick={onClick} disabled={pending || done} className="inline-flex items-center gap-1 text-[12px] font-semibold text-[var(--ui-muted)] hover:text-[var(--ui-ink)] disabled:opacity-50">
