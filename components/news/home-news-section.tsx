  ...
  return (
    ...
    >
      ...
      <div className={hasThumbnail ? "mt-2.5 sm:mt-3.5" : ""}>
        <h3 className="font-paperozi line-clamp-2 text-[15px] font-black leading-[1.4] tracking-[-0.035em] text-[var(--ui-ink)] group-hover:underline min-[390px]:text-[16px] sm:text-[19px]">
          {article.title}
        </h3>
...
export function HomeNewsSection({ articles }: { articles: NewsArticle[] }) {
  ...
  return (
    <section aria-labelledby="home-news-heading">
      ...
      <div className={`grid gap-4 rounded-xl border border-[var(--ui-card-divider)] p-2.5 min-[390px]:gap-5 min-[390px]:rounded-2xl min-[390px]:p-3 sm:p-4 ${lead ? "lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,1fr)] lg:gap-8" : ""}`}>
