...
import { type CSSProperties, useActionState, useEffect, useMemo, useState, useTransition } from "react";
  ...
  return (
    ...
    >
      <section
        ...
        className="onboarding-dialog-panel flex max-h-[92dvh] w-full flex-col overflow-hidden rounded-t-[24px] border border-b-0 border-[var(--ui-border)] bg-[var(--ui-surface)] shadow-[0_24px_80px_rgba(0,0,0,0.42)] transition-[max-width] sm:my-auto sm:rounded-2xl sm:border-b"
        style={{ "--onboarding-dialog-max-width": `${step === 1 ? 480 : 680}px` } as CSSProperties}
      >
        <DialogSheetHandle />
        ...
        {step === 1 ? (
          <form action={profileAction} className="min-h-0 overflow-y-auto px-5 pb-6 pt-6 sm:px-6">
            <h1 id="onboarding-title" className="font-paperozi text-center text-[24px] leading-[1.35] text-[var(--ui-ink)]">
              MINION에서 사용할 이름
            </h1>

            <label className="group relative mx-auto mt-6 block w-fit cursor-pointer" aria-label="프로필 사진 선택">
              <RankAvatar tier="bronze" src={previewUrl} alt="선택한 프로필 이미지" fallback={initials} size="profile" />
              <span
                className="absolute grid h-8 w-8 place-items-center rounded-full border-[3px] border-[var(--ui-surface)] bg-[var(--ui-ink)] text-[var(--ui-surface)] shadow-md transition group-hover:scale-105"
                style={{ right: -4, top: -4 }}
              >
                <Camera size={15} />
              </span>
            ...
            <div className="mt-6 flex flex-col gap-2">
              ...
              <input
                ...
                className="min-h-12 rounded-xl border border-[var(--ui-border)] bg-[var(--ui-surface-muted)] px-4 text-base text-[var(--ui-ink)] outline-none transition placeholder:text-[var(--ui-muted)] focus:border-[var(--accent)] focus:bg-[var(--ui-surface)] focus:ring-[3px] focus:ring-[color-mix(in_srgb,var(--accent)_18%,transparent)]"
              />
            ...
            <button type="submit" disabled={profilePending || nickname.trim().length < 2} className="mt-5 flex min-h-12 w-full items-center justify-center rounded-xl bg-[var(--accent)] px-5 text-sm font-medium text-[var(--accent-foreground)] shadow-[0_8px_24px_color-mix(in_srgb,var(--accent)_22%,transparent)] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-45">
          ...
          <div className="min-h-0 overflow-y-auto px-5 py-5 sm:px-6">
                ...
                return (
                  <li key={team.id}>
                    <button
                      ...
                      className={`group relative flex min-h-24 w-full flex-col items-center justify-center gap-1.5 rounded-xl border px-2 py-2 transition disabled:opacity-55 ${selected ? "border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_9%,var(--ui-surface))] ring-2 ring-[color-mix(in_srgb,var(--accent)_18%,transparent)]" : "border-[var(--ui-border)] bg-[var(--ui-surface)] hover:border-[var(--accent)] hover:bg-[var(--ui-card-hover)]"}`}
            ...
            <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                ...
                className="flex min-h-11 items-center justify-center rounded-xl bg-[var(--accent)] px-5 text-sm font-medium text-[var(--accent-foreground)] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-45"
              >
