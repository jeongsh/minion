  ...
  return (
    <Link href="/predictions" className={`grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center rounded-xl border border-[var(--ui-border)] bg-[var(--ui-surface)] transition hover:border-[var(--ui-ink)] ${compact ? "h-14 px-3" : "h-[76px] px-4"}`}>
      <div className="flex min-w-0 items-center gap-2">
        ...
        <span className="flex min-w-0 items-center gap-1.5"><span className="truncate text-sm font-black text-[var(--ui-ink)]">{teamA?.shortName ?? "TBD"}</span><small className="shrink-0 text-xs font-bold text-[var(--ui-muted)]">{oddsLabel(market.teamAOdds)}</small></span>
        <strong className={`ml-auto whitespace-nowrap ${compact ? "text-lg" : "text-2xl"}`}>{market.teamAPercent}%</strong>
      </div>
      <span className="px-3 text-[13px] font-black text-[var(--ui-muted)]">VS</span>
      <div className="flex min-w-0 flex-row-reverse items-center gap-2 text-right">
        ...
        <span className="flex min-w-0 flex-row-reverse items-center gap-1.5"><span className="truncate text-sm font-black text-[var(--ui-ink)]">{teamB?.shortName ?? "TBD"}</span><small className="shrink-0 text-xs font-bold text-[var(--ui-muted)]">{oddsLabel(market.teamBOdds)}</small></span>
