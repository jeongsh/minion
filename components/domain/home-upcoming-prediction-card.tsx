  ...
  return <>
    <article className="h-[154px] overflow-hidden rounded-xl bg-[#fafafa] p-3 dark:border dark:border-[#e3e1e8]">
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <button type="button" disabled={!teamA || pending || closed} onClick={() => teamA && open(match.id, teamA.id, teamA.shortName)} className="w-full rounded-lg border border-[#dcd9e2] py-2 text-xs font-black hover:border-[#1c192b] hover:bg-[#f1eff5] disabled:opacity-50">{teamA?.shortName ?? "TBD"} 승리 예측</button>
        <button type="button" disabled={!teamB || pending || closed} onClick={() => teamB && open(match.id, teamB.id, teamB.shortName)} className="w-full rounded-lg border border-[#dcd9e2] py-2 text-xs font-black hover:border-[#1c192b] hover:bg-[#f1eff5] disabled:opacity-50">{teamB?.shortName ?? "TBD"} 승리 예측</button>
      </div>
    </article>
    {modal}
  </>;
