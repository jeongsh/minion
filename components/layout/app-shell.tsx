...
export function AppShell({ children, currentUser = null }: { children: React.ReactNode; currentUser?: AppShellUser }) {
  ...
  useEffect(() => {
    document.documentElement.style.setProperty("--shell-lnb-width", collapsed ? "76px" : "240px");
  }, [collapsed]);
  ...
  return (
    <div className="min-h-screen text-[#141517]">
      <header className="fixed inset-x-0 top-0 z-50 flex h-16 items-center border-b border-[#e8e8eb] bg-background px-4">
        ...
        <label className="mx-auto hidden h-10 w-[360px] items-center gap-2 rounded-xl bg-[#f2f3f5] px-4 text-[#777b82] md:flex">
          <Search size={18} /><input className="min-w-0 flex-1 bg-transparent text-sm outline-none" placeholder="팀, 선수, 대회 검색" />
        </label>
      ...
      <div className={`flex min-h-screen flex-col pt-16 transition-[padding] ${collapsed ? "lg:pl-[76px]" : "lg:pl-[240px]"}`}><div className="flex-1 bg-[var(--ui-surface)]">{children}</div><SiteFooter /></div>
