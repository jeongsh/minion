...
export function FanPredictionCard({ matchId, teamId, teamName, opponentId, opponentName, teamColor, bets, currentUserId, balance, canVote, showDetailsLink = true }: { matchId?: string; teamId: string; teamName: string; opponentId?: string; opponentName: string; teamColor: string; bets: PredictionBet[]; currentUserId?: string; balance: number | null; canVote: boolean; showDetailsLink?: boolean }) {
  ...
  return <>
    <div className="flex flex-col gap-[14px] rounded-xl border border-black/10 bg-white px-6 py-[22px]" style={{color:"#16151b"}}>
      <div className="flex items-center justify-between"><span className="text-[13px] font-medium">승부예측</span><span className="text-[13px] font-medium text-[#9c9aa3]">{total > 0 ? `${total.toLocaleString()} LP 참여 중` : "가장 먼저 예측해보세요"}</span></div>
      <div className="flex flex-col gap-[7px]"><div className="flex h-3 overflow-hidden rounded-full bg-[#eeece8]"><span style={{width:`${teamPct}%`,background:teamColor}}/></div><div className="flex justify-between text-[13px] font-black"><span style={{color:teamColor}}>{teamName} {teamPct}%</span><span className="text-[#9c9aa3]">{opponentName} {oppPct}%</span></div></div>
