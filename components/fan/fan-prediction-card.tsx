...
export function FanPredictionCard({ matchId, teamId, teamName, opponentId, opponentName, teamColor, bets, currentUserId, balance, canVote, showDetailsLink = true, glass = false }: { matchId?: string; teamId: string; teamName: string; opponentId?: string; opponentName: string; teamColor: string; bets: PredictionBet[]; currentUserId?: string; balance: number | null; canVote: boolean; showDetailsLink?: boolean; glass?: boolean }) {
  ...
  return <>
    <div className={`flex min-w-0 flex-col gap-2.5 rounded-xl border px-3 py-3 sm:gap-[14px] sm:px-6 sm:py-[22px] ${shell}`} style={glass ? undefined : {color:"#16151b"}}>
      <div className="flex min-w-0 items-center justify-between gap-3"><span className={`shrink-0 text-[13px] font-medium ${glass ? "text-white" : ""}`}>승부예측</span><span className={`min-w-0 truncate text-right text-[12px] font-medium sm:text-[13px] ${muted}`}>{total > 0 ? `${total.toLocaleString()} LP 참여 중` : "가장 먼저 예측해보세요"}</span></div>
