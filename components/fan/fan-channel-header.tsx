...
export async function FanChannelHeader({ teamSlug }: { teamSlug: string; fanTemperature: FanTemperatureSnapshot }) {
  ...
  return (
    <header className="relative overflow-hidden bg-[var(--team-primary)] text-[var(--team-on-primary)]">
      <div className="relative mx-auto max-w-[1400px] px-5"><span aria-hidden className="pointer-events-none absolute -right-2 -top-16 text-[250px] font-black leading-none text-white/[0.07]">{team.shortName}</span>
        <div className="relative grid items-center gap-11 pb-10 pt-11 lg:grid-cols-[1fr_380px]">
          <div className="flex flex-col gap-[18px]">
            <div className="flex items-center gap-3"><span className="rounded bg-[var(--team-on-primary)] px-[10px] py-1 text-[13px] font-black text-[var(--team-primary)]">{badge}</span><span className="text-[13px] font-extrabold tracking-[0.12em] opacity-80">{match?.name?.trim() || "다가올 경기를 기다리는 중"}</span></div>
            <div className="flex items-baseline gap-5"><span className="text-[clamp(58px,7vw,96px)] font-black leading-[0.92] tracking-[-0.04em]">{team.shortName}</span><span className="text-[28px] font-black text-white/55">VS</span><span className="text-[clamp(58px,7vw,96px)] font-black leading-[0.92] tracking-[-0.04em] text-transparent [-webkit-text-stroke:2px_rgba(255,255,255,0.85)]">{opponentName}</span></div>
            <p className="text-[15px] font-bold opacity-85">{match ? `${dateTime(match.matchDate)} · ${match.venue?.trim() || "LoL PARK"}` : "예정된 경기가 없습니다"}</p>
            <div className="flex flex-wrap items-center gap-2"><FanFollowButton teamId={team.id} teamSlug={team.fanSiteHost} teamName={team.shortName} initialCount={fanCount} initialFollowing={isFan} teamColor={team.primaryColor}/><button type="button" className="rounded-full border border-white/50 px-5 py-2.5 text-sm font-extrabold transition hover:bg-white/10">알람</button>{links.map(([label,href])=><a key={label} href={href} target="_blank" rel="noreferrer" className="rounded-full border border-white/35 px-4 py-2.5 text-[13px] font-bold text-white/85 hover:bg-white/10 hover:text-white">{label}</a>)}</div>
          </div>
      ...
      <div className="overflow-hidden bg-black/20 py-[9px]"><div className="fan-ticker-track text-[13px] font-extrabold tracking-[0.08em] text-white/85">{[false,true].map((hidden)=><div key={String(hidden)} className="fan-ticker-group" aria-hidden={hidden||undefined}>{tickerItems.map((item)=><span key={item} className="px-5">{item}<span className="ml-10 text-white/45">•</span></span>)}</div>)}</div></div>
    </header>
  );
}
