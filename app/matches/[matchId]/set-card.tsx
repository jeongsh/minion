  ...
  return (
    <div className="relative h-14 w-14 flex-shrink-0 overflow-hidden rounded border border-border bg-background" title={championLabel(champion)}>
      {img ? (
        ...
        <div className="absolute inset-0 flex items-center justify-center text-[13px] text-muted">-</div>
      )}
  ...
  return (
    <div className={`flex flex-col gap-2 ${flip ? "items-end" : ""}`}>
      <p className={`flex items-center gap-1.5 text-sm font-semibold ${flip ? "flex-row-reverse text-red-500" : "text-blue-500"}`}>
        <span>{teamName}</span>
        {won && <span className="rounded bg-accent px-1.5 py-0.5 text-[13px] font-bold text-accent-foreground">승</span>}
      </p>
    ...
    <div className="overflow-x-auto">
      <div className={`${ROW_GRID} items-center gap-2 px-3 py-2 ${headerBg}`}>
        <span className={`text-sm font-bold ${headerText}`}>
          ...
          {won && <span className="ml-1.5 rounded bg-accent px-1.5 py-0.5 text-[13px] font-bold text-accent-foreground">승</span>}
        </span>
        <span className="text-[13px] font-semibold uppercase text-muted">KDA</span>
        <span className="text-[13px] font-semibold uppercase text-muted">딜량</span>
        <span className="text-[13px] font-semibold uppercase text-muted">골드</span>
        <span className="text-center text-[13px] font-semibold uppercase text-muted">시야</span>
        <span className="text-center text-[13px] font-semibold uppercase text-muted">CS</span>
        <span className="text-[13px] font-semibold uppercase text-muted">아이템</span>
      </div>
      {teamLines.map((line) => (
        <PlayerRow
          key={line.playerId}
...
function DualPlayerChart({
  ...
  return (
    <div className="border-b border-border">
      <p className="pb-1.5 text-center text-[13px] font-bold uppercase tracking-widest text-muted">
        딜량 (DAMAGE)
      </p>
      ...
      <div className={`${GRID} items-center gap-x-1 border-t border-border/40 px-4 py-1`}>
        <span /><span />
        <span className="text-right text-[13px] font-semibold uppercase text-muted">선수</span>
        <span className="text-right text-[13px] font-semibold uppercase text-muted">CS · 골드</span>
        <span className="text-right text-[13px] font-semibold uppercase text-muted">아이템</span>
        <span />
        <span />
        <span className="text-left text-[13px] font-semibold uppercase text-muted">아이템</span>
        <span className="text-left text-[13px] font-semibold uppercase text-muted">골드 · CS</span>
        <span className="text-left text-[13px] font-semibold uppercase text-muted">선수</span>
        <span /><span />
      </div>
          ...
          return (
            <div key={i} className={`${GRID} items-center gap-x-1 border-t border-border/40 px-4 py-2`}>
              ...
              <div className="text-right">
                <p className="truncate text-[13px] font-semibold">{bPlayer?.name ?? "-"}</p>
                <p className="truncate text-[13px] text-muted">{champions.find((c) => c.id === bl?.championId)?.name ?? ""}</p>
              </div>

              {/* 블루 CS */}
              <div className="text-right">
                <p className="text-[13px] tabular-nums font-semibold">{bl ? `${bl.cs} CS` : "-"}</p>
                <p className="text-[13px] tabular-nums text-muted">{bl ? goldVal(bl) : ""}</p>
              </div>
              ...
              <div className="flex h-5 items-center justify-end gap-1.5 pr-px">
                <span className="shrink-0 text-[13px] font-semibold tabular-nums text-blue-400">{fmtVal(bv)}</span>
              ...
              <div className="flex h-5 items-center gap-1.5 pl-px">
                ...
                <span className="shrink-0 text-[13px] font-semibold tabular-nums text-red-400">{fmtVal(rv)}</span>
              </div>
              ...
              <div className="text-left">
                <p className="text-[13px] tabular-nums font-semibold">{rl ? goldVal(rl) : "-"}</p>
                <p className="text-[13px] tabular-nums text-muted">{rl ? `${rl.cs} CS` : ""}</p>
              </div>

              {/* 레드 이름 */}
              <div className="text-left">
                <p className="truncate text-[13px] font-semibold">{rPlayer?.name ?? "-"}</p>
                <p className="truncate text-[13px] text-muted">{champions.find((c) => c.id === rl?.championId)?.name ?? ""}</p>
              </div>
  ...
  return (
    <div className="overflow-hidden rounded-md border border-border bg-surface">
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-2.5">
        ...
        <span className="text-[13px] text-muted">{durationLabel(set.durationSeconds)}</span>
        {!hasPickBan && <span className="text-[13px] text-muted">밴픽 데이터 없음</span>}
        {hasStats && (
          <button
            type="button"
      ...
      {hasPickBan ? (
        <div className="grid grid-cols-[1fr_minmax(160px,200px)_1fr] items-center gap-3 p-4">
          ...
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-1">
              ...
              <span className="px-2 text-center text-[13px] font-semibold text-muted">KILLS</span>
            ...
            {(blueDamage > 0 || redDamage > 0) && (
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-1">
                ...
                <span className="px-2 text-center text-[13px] font-semibold text-muted">딜량</span>
            ...
            {(blueGold > 0 || redGold > 0) && (
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-1">
                ...
                <span className="px-2 text-center text-[13px] font-semibold text-muted">골드</span>
            ...
            <div className="grid grid-cols-2 gap-x-3 gap-y-1">
              {[
                ...
                {
                  label: "포탑",
                  blue: <span className="text-[13px] font-bold tabular-nums text-blue-400">{set.blueTowers ?? "-"}</span>,
                  red: <span className="text-[13px] font-bold tabular-nums text-red-400">{set.redTowers ?? "-"}</span>,
                },
                {
                  label: "공허충",
        ...
        <div className="p-4 text-[13px] text-muted">데이터 없음</div>
      )}
        ...
        <div className="border-t border-border">
          ...
          {timelineEvents && timelineEvents.length > 0 && (
            <div className="border-y border-border bg-surface-muted px-4 py-3">
              <p className="mb-2 text-[13px] font-semibold uppercase text-muted">타임라인</p>
              <GameTimeline
