  ...
  return (
    <div className="relative h-14 w-14 flex-shrink-0 overflow-hidden rounded border border-border bg-background" title={championLabel(champion)}>
      {img ? (
        ...
        <div className="absolute inset-0 flex items-center justify-center text-[15px] text-muted">-</div>
      )}
...
function SideDraft({
  ...
  return (
    <div className={`flex flex-col gap-2 ${flip ? "items-end" : ""}`}>
      <p className={`flex items-center gap-1.5 text-[15px] font-semibold ${flip ? "flex-row-reverse text-red-500" : "text-blue-500"}`}>
        <span>{teamName}</span>
        {won && <span className="rounded bg-accent px-1.5 py-0.5 text-[15px] font-bold text-accent-foreground">승</span>}
      </p>
...
function SpellIcon({ src }: { src: string }) {
  return (
    <span className="relative block h-8 w-8 shrink-0 overflow-hidden rounded-sm border border-border/60 bg-surface-muted">
...
function RuneIcon({ src, isTree = false }: { src: string; isTree?: boolean }) {
  const className = isTree ? "h-4 w-4" : "h-8 w-8 border border-white/10 bg-[#0d1117]";
  return (
    <span className={`relative block shrink-0 overflow-hidden rounded-full ${className}`}>
      {src ? <Image src={src} alt="" fill sizes={isTree ? "16px" : "32px"} unoptimized className="object-contain" /> : null}
    </span>
  );
}
...
function PlayerRow({
  ...
  return (
    <div className="grid min-w-[920px] grid-cols-[200px_1fr_120px_100px_48px_65px_250px] items-center gap-2 border-t border-border px-3 py-2.5">
      ...
      <div>
        <p className="text-[15px] font-semibold tabular-nums">{line.damageToChampions.toLocaleString("ko-KR")}</p>
        <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-surface-muted">
          <div className={`h-full rounded-full ${barColor}`} style={{ width: `${dmgPct}%` }} />
        </div>
      </div>
      <div>
        <p className="text-[15px] font-semibold tabular-nums">{line.gold >= 1000 ? `${(line.gold / 1000).toFixed(1)}K` : line.gold}</p>
        <div className="mt-1 h-1.5 w-full overflow-hidden rounded-full bg-surface-muted">
  ...
  return (
    <div className="overflow-x-auto">
      <div className={`${ROW_GRID} items-center gap-2 px-3 py-2 ${headerBg}`}>
        <span className={`text-[15px] font-bold ${headerText}`}>
          {team?.shortName ?? (side === "blue" ? "블루" : "레드")}
          {won && <span className="ml-1.5 rounded bg-accent px-1.5 py-0.5 text-[15px] font-bold text-accent-foreground">승</span>}
        </span>
        <span className="text-[15px] font-semibold uppercase text-muted">KDA</span>
        <span className="text-[15px] font-semibold uppercase text-muted">딜량</span>
        <span className="text-[15px] font-semibold uppercase text-muted">골드</span>
        <span className="text-center text-[15px] font-semibold uppercase text-muted">시야</span>
        <span className="text-center text-[15px] font-semibold uppercase text-muted">CS</span>
        <span className="text-[15px] font-semibold uppercase text-muted">아이템</span>
      </div>
      {teamLines.map((line) => (
        <PlayerRow
          key={line.playerId}
...
function DualPlayerChart({
  ...
  return (
    <div className="border-b border-border">
      <p className="pb-1.5 text-center text-[15px] font-bold uppercase tracking-widest text-muted">
        딜량 (DAMAGE)
      </p>
      ...
      <div className={`${GRID} items-center gap-x-1 border-t border-border/40 px-4 py-1`}>
        <span /><span />
        <span className="text-right text-[15px] font-semibold uppercase text-muted">선수</span>
        <span className="text-right text-[15px] font-semibold uppercase text-muted">CS · 골드</span>
        <span className="text-right text-[15px] font-semibold uppercase text-muted">아이템</span>
        <span />
        <span />
        <span className="text-left text-[15px] font-semibold uppercase text-muted">아이템</span>
        <span className="text-left text-[15px] font-semibold uppercase text-muted">골드 · CS</span>
        <span className="text-left text-[15px] font-semibold uppercase text-muted">선수</span>
        <span /><span />
      </div>
          ...
          return (
            <div key={i} className={`${GRID} items-center gap-x-1 border-t border-border/40 px-4 py-2`}>
              ...
              <div className="text-right">
                <p className="truncate text-[15px] font-semibold">{bPlayer?.name ?? "-"}</p>
                <p className="truncate text-[15px] text-muted">{champions.find((c) => c.id === bl?.championId)?.name ?? ""}</p>
              </div>

              {/* 블루 CS */}
              <div className="text-right">
                <p className="text-[15px] tabular-nums font-semibold">{bl ? `${bl.cs} CS` : "-"}</p>
                <p className="text-[15px] tabular-nums text-muted">{bl ? goldVal(bl) : ""}</p>
              </div>
              ...
              <div className="flex h-5 items-center justify-end gap-1.5 pr-px">
                <span className="shrink-0 text-[15px] font-semibold tabular-nums">{fmtVal(bv)}</span>
              ...
              <div className="flex h-5 items-center gap-1.5 pl-px">
                ...
                <span className="shrink-0 text-[15px] font-semibold tabular-nums">{fmtVal(rv)}</span>
              </div>
              ...
              <div className="text-left">
                <p className="text-[15px] tabular-nums font-semibold">{rl ? goldVal(rl) : "-"}</p>
                <p className="text-[15px] tabular-nums text-muted">{rl ? `${rl.cs} CS` : ""}</p>
              </div>

              {/* 레드 이름 */}
              <div className="text-left">
                <p className="truncate text-[15px] font-semibold">{rPlayer?.name ?? "-"}</p>
                <p className="truncate text-[15px] text-muted">{champions.find((c) => c.id === rl?.championId)?.name ?? ""}</p>
              </div>
  ...
  return (
    <div className="overflow-hidden rounded-md border border-border bg-surface">
      ...
      <div className="flex items-center gap-3 border-b border-border px-4 py-2.5">
        ...
        <span className="text-[15px] text-muted">{durationLabel(set.durationSeconds)}</span>
        {!hasPickBan && <span className="text-[15px] text-muted">밴픽 데이터 없음</span>}
        {hasStats && (
          <button
            type="button"
      ...
      {hasPickBan ? (
        <div className="grid grid-cols-[1fr_minmax(160px,200px)_1fr] items-center gap-3 p-4">
          ...
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-1">
              ...
              <span className="px-2 text-center text-[15px] font-semibold text-muted">KILLS</span>
              <span className="text-left text-2xl font-bold tabular-nums text-red-500">{set.redKills ?? "-"}</span>
            </div>
            {(blueDamage > 0 || redDamage > 0) && (
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-1">
                <span className="text-right text-[15px] font-semibold tabular-nums text-blue-400">{blueDamage >= 1000 ? `${(blueDamage / 1000).toFixed(1)}K` : blueDamage}</span>
                <span className="px-2 text-center text-[15px] font-semibold text-muted">딜량</span>
                <span className="text-left text-[15px] font-semibold tabular-nums text-red-400">{redDamage >= 1000 ? `${(redDamage / 1000).toFixed(1)}K` : redDamage}</span>
              </div>
            )}
            {(blueGold > 0 || redGold > 0) && (
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-1">
                <span className="text-right text-[15px] font-semibold tabular-nums text-blue-400">{blueGold >= 1000 ? `${(blueGold / 1000).toFixed(1)}K` : blueGold}</span>
                <span className="px-2 text-center text-[15px] font-semibold text-muted">골드</span>
                <span className="text-left text-[15px] font-semibold tabular-nums text-red-400">{redGold >= 1000 ? `${(redGold / 1000).toFixed(1)}K` : redGold}</span>
              </div>
            )}
            <div className="grid grid-cols-2 gap-x-3 gap-y-1">
              {[
                ...
                {
                  label: "포탑",
                  blue: <span className="text-[15px] font-bold tabular-nums text-blue-400">{set.blueTowers ?? "-"}</span>,
                  red: <span className="text-[15px] font-bold tabular-nums text-red-400">{set.redTowers ?? "-"}</span>,
                },
                {
                  label: "공허충",
                ...
                <div key={label} className="grid grid-cols-[auto_1fr_auto] items-center gap-1">
                  ...
                  <span className="text-center text-[15px] font-semibold text-muted">{label}</span>
        ...
        <div className="p-4 text-[15px] text-muted">데이터 없음</div>
      )}
        ...
        <div className="border-t border-border">
          ...
          {timelineEvents && timelineEvents.length > 0 && (
            <div className="border-y border-border bg-surface-muted px-4 py-3">
              <p className="mb-2 text-[15px] font-semibold uppercase text-muted">타임라인</p>
              <GameTimeline
