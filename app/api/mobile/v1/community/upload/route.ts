...
export async function POST(request: Request) {
  ...
  const guest = auth ? null : (() => { try { return getMobileGuestIdentity(request); } catch { return null; } })();
  if (!auth && !guest) return mobileError("BAD_REQUEST", "비회원 ID를 확인하지 못했습니다.", 400);
  const ownerKey = auth?.user.id ?? guest!.key;
  ...
  try {
    ...
    const objectPath = `${communityUploadPrefix(auth?.user.id ?? `guest-${guest!.key.slice(0, 32)}`)}/${crypto.randomUUID()}.${prepared.extension}`;
    ...
    await recordOperationalEvent(admin, {
      ...
      metadata: { bucket: COMMUNITY_UPLOAD_BUCKET, contentType: prepared.contentType, guest: Boolean(guest), originalBytes: bytes.byteLength, storedBytes: prepared.bytes.byteLength, transformed: prepared.transformed },
      targetId: objectPath,
