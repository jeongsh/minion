  ...
  return (
    <main className="min-h-screen text-[var(--ui-text)]">
      <div className="layout-wide pb-20 pt-6 sm:pt-10">
        ...
        <p className="mt-4 max-w-2xl text-xs leading-5 text-[var(--ui-muted)] md:text-[13px] md:leading-[1.625rem] lg:text-sm lg:leading-6">MINION이 어떤 기준으로 서비스를 운영하고 정보를 처리하는지 확인할 수 있습니다.</p>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          {POLICIES.map(({ href, title, description, icon: Icon }) => (
            <Link key={href} href={href} className="group rounded-2xl border border-[var(--ui-border)] bg-[var(--ui-surface)] p-5 transition hover:-translate-y-0.5 hover:bg-[var(--ui-surface-muted)]">
              ...
              <h2 className="mt-5 text-base font-black text-[var(--ui-ink)] md:text-[17px] lg:text-lg">{title}</h2>
              <p className="mt-2 text-xs leading-5 text-[var(--ui-muted)] md:text-[13px] md:leading-[1.625rem] lg:text-sm lg:leading-6">{description}</p>
              <span className="mt-5 inline-block text-xs font-medium text-[var(--ui-ink)] md:text-[13px] lg:text-sm">확인하기 →</span>
            </Link>
          ))}
        </div>
      </div>
