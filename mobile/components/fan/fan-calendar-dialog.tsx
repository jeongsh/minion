...
export function FanCalendarDialog({ background, border, events = [], iconColor, matches, teamShort }: Props) {
  ...
  return <>
    ...
    <BottomSheet contentStyle={styles.body} maxHeight="92%" onClose={() => setOpen(false)} open={open} title={`${teamShort} 캘린더`}>
            <View style={[styles.calendar, { backgroundColor: colorScheme === 'dark' ? '#1c1e22' : '#ffffff', borderColor: theme.border }]}>
              {selectedKey && (selectedMatches.length || selectedEvents.length) ? (
                ...
                <View style={styles.week}>{WEEKDAYS.map((day) => <Text key={day} style={[styles.weekday, { color: theme.muted, ...fonts.medium }]}>{day}</Text>)}</View>
...
function CalendarDetail({ dateKey, events, matches, onBack }: { dateKey: string; events: MobileTeamDetailDto['calendarEvents']; matches: MobileTeamDetailDto['matches']; onBack: () => void }) {
  ...
  return <View style={styles.detail}>
    <View style={styles.detailHeader}><Pressable onPress={onBack} style={styles.detailBack}><ChevronLeft color={theme.ink} size={16} strokeWidth={2.5} /><Text style={{ color: theme.ink, ...fonts.black, fontSize: 14 }}>{month}월 {day}일</Text></Pressable><Pressable accessibilityLabel="날짜 상세 닫기" onPress={onBack} style={styles.detailClose}><X color={theme.muted} size={14} strokeWidth={2.5} /></Pressable></View>
    <ScrollView contentContainerStyle={styles.detailList}>{matches.map((match) => <View key={match.id} style={[styles.match, { backgroundColor: theme.card }]}><View><Text style={{ color: theme.muted, ...fonts.medium, fontSize: 12 }}>{match.tournament?.league ?? ''}</Text><View style={styles.time}><Clock3 color="#00b979" size={12} /><Text style={{ color: '#00b979', ...fonts.medium, fontSize: 12 }}>{matchTime(match.startsAt)}</Text></View></View><View style={styles.teams}><TeamLogo plain size={16} team={match.teamA} themeAware /><Text style={{ color: theme.ink, ...fonts.black, fontSize: 13 }}>{match.teamA?.shortName ?? 'TBD'}</Text><Text style={{ color: theme.muted, ...fonts.medium, fontSize: 12 }}>vs</Text><TeamLogo plain size={16} team={match.teamB} themeAware /><Text style={{ color: theme.ink, ...fonts.black, fontSize: 13 }}>{match.teamB?.shortName ?? 'TBD'}</Text></View></View>)}{events.map((event) => { const imageUrl = resolveApiAssetUrl(event.image?.url); return <View key={event.id} style={[styles.event, { backgroundColor: theme.card }]}>{imageUrl ? <Image contentFit="cover" contentPosition="top" source={{ uri: imageUrl }} style={styles.eventImage} /> : <View style={[styles.eventIcon, { backgroundColor: `${EVENT_COLOR[event.type]}1f` }]}><Text>{event.type === 'birthday' ? '🎂' : event.type === 'debut' ? '🎉' : event.type === 'championship' ? '🏆' : '🎈'}</Text></View>}<Text numberOfLines={1} style={{ color: theme.ink, flex: 1, ...fonts.black, fontSize: 13 }}>{event.title}</Text><Text style={{ color: EVENT_COLOR[event.type], ...fonts.medium, fontSize: 12 }}>{event.dday === 0 ? 'D-DAY' : `D-${event.dday}`}</Text></View>; })}</ScrollView>
  </View>;
}

function Legend({ color, label }: { color: string; label: string }) {
  const { fonts, theme } = useMinionTheme();
  return <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: color }]} /><Text style={{ color: theme.muted, ...fonts.medium, fontSize: 12 }}>{label}</Text></View>;
}
