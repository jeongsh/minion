...
export function HomeCalendarDialog({ calendar, events }: Props) {
  ...
  return <>
    <Pressable accessibilityLabel="LCK 캘린더 열기" onPress={() => setOpen(true)} style={[styles.trigger, { backgroundColor: theme.card }]}>
      ...
      <Text style={[styles.triggerText, { color: theme.ink, ...fonts.bold }]}>월간 캘린더 보기</Text>
    </Pressable>
    <BottomSheet contentStyle={styles.dialogBody} maxHeight="92%" onClose={() => setOpen(false)} open={open} title="LCK 캘린더">
            <View style={[styles.calendarShell, { backgroundColor: colorScheme === 'dark' ? '#1c1e22' : '#ffffff', borderColor: theme.border }]}>
              {selectedKey && (selectedMatches.length || selectedEvents.length) ? (
                ...
                <View style={styles.weekRow}>{WEEKDAYS.map((weekday) => <Text key={weekday} style={[styles.weekday, { color: theme.muted, ...fonts.medium }]}>{weekday}</Text>)}</View>
...
function CalendarDetail({ dateKey, events, matches, onBack }: { dateKey: string; events: MobileHomeDto['calendarEvents']; matches: MobileMatchSummary[]; onBack: () => void }) {
  ...
  return <View style={styles.detail}><View style={styles.detailHeader}><Pressable onPress={onBack} style={styles.detailBack}><ChevronLeft color={theme.ink} size={16} strokeWidth={2.5} /><Text style={{ color: theme.ink, ...fonts.black, fontSize: 14, lineHeight: 17.5 }}>{month}월 {day}일</Text></Pressable><Pressable accessibilityLabel="날짜 상세 닫기" onPress={onBack} style={styles.detailClose}><X color={theme.muted} size={14} strokeWidth={2.5} /></Pressable></View><ScrollView contentContainerStyle={styles.detailList}>{matches.map((match) => <View key={match.id} style={[styles.matchDetail, { backgroundColor: theme.card }]}><View style={styles.matchMeta}><Text style={{ color: theme.muted, ...fonts.medium, fontSize: 13, lineHeight: 19.5 }}>{match.tournament?.league ?? ''}</Text><View style={styles.matchTime}><Clock3 color="#00b979" size={12} strokeWidth={2.25} /><Text style={{ color: '#00b979', ...fonts.medium, fontSize: 13, lineHeight: 19.5 }}>{timeOf(match.startsAt)}</Text></View></View><View style={styles.detailTeams}><TeamLogo plain size={16} team={match.teamA} themeAware /><Text style={{ color: theme.ink, ...fonts.black, fontSize: 14, lineHeight: 17.5 }}>{match.teamA?.shortName ?? 'TBD'}</Text><Text style={{ color: theme.muted, ...fonts.medium, fontSize: 13, lineHeight: 19.5 }}>vs</Text><TeamLogo plain size={16} team={match.teamB} themeAware /><Text style={{ color: theme.ink, ...fonts.black, fontSize: 14, lineHeight: 17.5 }}>{match.teamB?.shortName ?? 'TBD'}</Text></View></View>)}{events.map((event) => <View key={event.id} style={[styles.eventDetail, { backgroundColor: theme.card }]}>{event.image?.url ? <Image contentFit="cover" contentPosition="top" source={{ uri: resolveApiAssetUrl(event.image.url) ?? undefined }} style={styles.eventImage} /> : <View style={[styles.eventIcon, { backgroundColor: `${EVENT_COLOR[event.type]}1f` }]}><Text>{event.type === 'birthday' ? '🎂' : event.type === 'debut' ? '🎉' : event.type === 'championship' ? '🏆' : '🎈'}</Text></View>}<Text numberOfLines={1} style={{ color: theme.ink, flex: 1, ...fonts.black, fontSize: 13, lineHeight: 19.5 }}>{event.title}</Text><Text style={{ color: EVENT_COLOR[event.type], ...fonts.medium, fontSize: 13, lineHeight: 19.5 }}>{event.dday === 0 ? 'D-DAY' : `D-${event.dday}`}</Text></View>)}</ScrollView></View>;
}

function Legend({ color, label }: { color: string; label: string }) {
  const { fonts, theme } = useMinionTheme();
  return <View style={styles.legendItem}><View style={[styles.legendDot, { backgroundColor: color }]} /><Text style={{ color: theme.muted, ...fonts.medium, fontSize: 13, lineHeight: 19.5 }}>{label}</Text></View>;
}
