  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.pageBackground }]}>
      <MinionScreen contentStyle={styles.content}>
        <Text style={[styles.pageTitle, { color: theme.ink, fontFamily: fonts.display }]}>사용자 활동</Text>
        {loading && !data ? <UserActivitySkeleton /> : error && !data ? <ErrorState onRetry={refresh} title={error} /> : data ? <>
          <View style={[styles.profileCard, { backgroundColor: theme.surface, borderColor: theme.border }]}>
            <Text style={[styles.eyebrow, { color: theme.muted, fontFamily: fonts.medium }]}>COMMUNITY PROFILE</Text>
            <CommunityAuthor author={data.profile} onBlocked={() => router.replace('/community' as never)} variant="profile" />
            <View style={styles.profileMeta}><Text style={[styles.tier, { backgroundColor: theme.surfaceMuted, color: theme.text, fontFamily: fonts.medium }]}>{TIER_LABELS[data.profile.tier ?? 'bronze'] ?? '브론즈'}</Text><Text style={{ color: theme.muted, fontFamily: fonts.medium, fontSize: 13 }}>{new Date(data.profile.createdAt).toLocaleDateString('ko-KR')} 가입</Text></View>
...
function ProfileStat({ label, value }: { label: string; value: number }) { const { fonts, theme } = useMinionTheme(); return <View style={styles.stat}><Text style={{ color: theme.muted, fontFamily: fonts.medium, fontSize: 13 }}>{label}</Text><Text style={{ color: theme.ink, fontFamily: fonts.bold, fontSize: 20, marginTop: 2 }}>{value}</Text></View>; }
function ActivityTab({ active, count, icon, label, onPress }: { active: boolean; count: number; icon: React.ReactNode; label: string; onPress: () => void }) { const { fonts, theme } = useMinionTheme(); return <Pressable accessibilityRole="tab" accessibilityState={{ selected: active }} onPress={onPress} style={[styles.tab, active ? { backgroundColor: theme.surface } : null]}>{icon}<Text style={{ color: active ? theme.ink : theme.muted, fontFamily: fonts.medium, fontSize: 14 }}>{label}</Text><Text style={{ color: active ? theme.text : theme.muted, fontFamily: fonts.regular, fontSize: 13 }}>{count}</Text></Pressable>; }
function Meta({ icon, value }: { icon: React.ReactNode; value: number }) { const { fonts, theme } = useMinionTheme(); return <View style={styles.metaItem}>{icon}<Text style={{ color: theme.muted, fontFamily: fonts.regular, fontSize: 13 }}>{value}</Text></View>; }
function EmptyActivity({ title }: { title: string }) { const { fonts, theme } = useMinionTheme(); return <View style={styles.empty}><Text style={{ color: theme.muted, fontFamily: fonts.regular, fontSize: 16 }}>{title}</Text></View>; }
...
const styles = StyleSheet.create({
  ...
  empty: { alignItems: 'center', justifyContent: 'center', minHeight: 116, padding: 20 },
  skeleton: { gap: 20 },
