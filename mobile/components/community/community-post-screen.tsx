  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={[styles.root, { backgroundColor: theme.pageBackground }]}>
      ...
      <View style={[styles.header, { borderBottomColor: theme.divider, marginTop: insets.top }]}>
        ...
        <Text numberOfLines={1} style={[styles.headerTitle, { color: theme.ink, fontFamily: fonts.display }]}>{team?.shortName ?? 'LCK'}</Text>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 56 + (Platform.OS === 'web' ? 0 : insets.bottom) }} keyboardShouldPersistTaps="handled" ref={scrollRef} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.article, { backgroundColor: theme.surface, borderTopColor: theme.border }]}>
          ...
          <View style={[styles.postActions, { borderColor: theme.divider }]}>
            ...
            <Pressable onPress={() => report('post', data.id)} style={[styles.report, { backgroundColor: theme.surface, borderColor: theme.border }]}><Flag color={theme.muted} size={15} strokeWidth={1.8} /><Text style={{ color: theme.muted, fontFamily: fonts.medium, fontSize: 13, lineHeight: 19.5 }}>리폿</Text></Pressable>
          </View>
          <View style={styles.commentTitle}><Text style={{ color: theme.ink, fontFamily: fonts.bold, fontSize: 15, lineHeight: 22.5 }}>댓글</Text><Text style={{ color: accent, fontFamily: fonts.medium, fontSize: 13, lineHeight: 19.5 }}>{data.commentCount}</Text></View>
              ...
              <View key={item.id} style={styles.commentThread}>
                ...
                {itemReplies.length > 0 && !repliesExpanded ? (
                  <View style={styles.threadToggleRow}>
                    ...
                    <Pressable accessibilityState={{ expanded: false }} onPress={() => toggleReplies(item.id)} style={styles.threadToggleButton}><Text style={{ color: theme.text, fontFamily: fonts.medium, fontSize: 14, lineHeight: 20 }}>답글 {itemReplies.length}개</Text><ChevronDown color={theme.text} size={18} strokeWidth={2} /></Pressable>
                  </View>
                ) : null}
                {itemReplies.length > 0 && repliesExpanded ? (
                  <View>
                    ...
                    <View style={styles.threadToggleRow}>
                      <View style={[styles.threadConnection, { borderColor: theme.border }]} />
                      <Pressable accessibilityState={{ expanded: true }} onPress={() => toggleReplies(item.id)} style={styles.threadToggleButton}><Text style={{ color: theme.text, fontFamily: fonts.medium, fontSize: 14, lineHeight: 20 }}>답글 숨기기</Text><ChevronUp color={theme.text} size={18} strokeWidth={2} /></Pressable>
                    </View>
                  </View>
                ) : null}
              </View>
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.pageBackground }]}>
      ...
      <View style={[styles.header, { borderBottomColor: theme.divider, marginTop: insets.top }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.ink, fontFamily: fonts.display }]}>{headerTitle}</Text>
...
function ReactionButton({ accent, active, count, icon, label, onPress, small = false }: { accent?: string; active: boolean; count: number; icon: 'honor' | 'dislike'; label: string; onPress: () => void; small?: boolean }) {
  ...
  return <Pressable accessibilityLabel={`${label} ${count}`} accessibilityState={{ selected: active }} onPress={onPress} style={[small ? styles.smallReaction : styles.reaction, small ? null : { borderColor: theme.border }]}><Icon color={active ? activeColor : small ? theme.muted : theme.text} size={16} strokeWidth={1.8} />{small ? <Text style={{ color: active ? activeColor : theme.muted, fontFamily: fonts.medium, fontSize: 12, lineHeight: 18 }}>{count}</Text> : <Text style={{ color: active ? activeColor : theme.text, fontFamily: fonts.medium, fontSize: 13, lineHeight: 19.5 }}>{label} {count}</Text>}</Pressable>;
}
    ...
    <View style={styles.commentItem}>
      ...
      <View style={styles.commentInner}>
        <View style={styles.commentTop}>
          ...
          <Text style={{ color: theme.muted, fontFamily: fonts.regular, fontSize: 12, lineHeight: 18 }}>{formatCommunityDate(comment.createdAt)}</Text>
        ...
        {comment.isDeleted ? (
          <Text style={[styles.commentBody, { color: theme.muted, fontFamily: fonts.regular, fontSize: 14 }]}>삭제된 댓글입니다.</Text>
        ) : editing ? (
          <View style={[styles.editBox, { borderColor: theme.border }]}>
            <TextInput maxLength={COMMENT_MAX_LENGTH} multiline onChangeText={setContent} style={{ color: theme.text, fontFamily: fonts.regular, fontSize: 15, lineHeight: 24.75, minHeight: 72 }} value={content} />
            <View style={styles.editActions}><Pressable onPress={() => setEditing(false)}><Text style={{ color: theme.muted, fontFamily: fonts.medium, fontSize: 13 }}>취소</Text></Pressable><Pressable disabled={!content.trim() || busy} onPress={() => void save()}><Text style={{ color: accent, fontFamily: fonts.medium, fontSize: 13 }}>{busy ? '저장 중' : '저장'}</Text></Pressable></View>
          </View>
        ) : (
          <Text style={[styles.commentBody, { color: comment.isBlinded ? theme.muted : theme.text, fontFamily: fonts.regular }]}>{comment.isBlinded ? '블라인드된 댓글입니다.' : String(comment.content)}</Text>
        )}
