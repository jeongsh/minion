...
export function CommunityRichEditor({ allowEmbeds, allowMedia, characterCount, characterLimit, maxImages, onChange, value }: Props) {
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.surface }]}>
      ...
      <BottomSheet onClose={closeLink} open={linkOpen} title={linkMode === 'youtube' ? 'YouTube 영상 넣기' : 'SNS 게시물 넣기'}><TextInput autoCapitalize="none" autoCorrect={false} onChangeText={setLinkUrl} onSubmitEditing={insertLink} placeholder={linkMode === 'youtube' ? 'YouTube URL을 붙여넣으세요' : 'Instagram 또는 X URL을 붙여넣으세요'} placeholderTextColor={theme.muted} returnKeyType="done" style={[styles.linkInput, { borderColor: theme.border, color: theme.text, fontFamily: fonts.regular }]} value={linkUrl} /><Pressable disabled={!linkUrl.trim()} onPress={insertLink} style={({ pressed }) => [styles.sheetPrimary, { backgroundColor: theme.ink, opacity: !linkUrl.trim() ? 0.4 : pressed ? 0.78 : 1 }]}><Text style={{ color: theme.surface, fontFamily: fonts.medium, fontSize: 14 }}>넣기</Text></Pressable></BottomSheet>
      <BottomSheet onClose={() => { setPollOpen(false); setPoll(emptyPoll()); }} open={pollOpen} title="투표 만들기">
        <Text style={{ color: theme.muted, fontFamily: fonts.medium, fontSize: 13, marginBottom: 14 }}>질문과 선택지를 입력해 주세요.</Text><Text style={[styles.fieldLabel, { color: theme.text, fontFamily: fonts.medium }]}>질문</Text><TextInput maxLength={100} onChangeText={(question) => setPoll((current) => ({ ...current, question }))} placeholder="무엇을 물어볼까요?" placeholderTextColor={theme.muted} style={[styles.pollInput, { borderColor: theme.border, color: theme.text, fontFamily: fonts.medium }]} value={poll.question} /><Text style={[styles.fieldLabel, { color: theme.text, fontFamily: fonts.medium, marginTop: 14 }]}>선택지</Text>
        <View style={styles.pollOptions}>{poll.options.map((option, index) => <View key={option.id} style={styles.pollOptionRow}><TextInput maxLength={40} onChangeText={(label) => setPoll((current) => ({ ...current, options: current.options.map((item) => item.id === option.id ? { ...item, label } : item) }))} placeholder={`선택지 ${index + 1}`} placeholderTextColor={theme.muted} style={[styles.pollInput, styles.pollOptionInput, { borderColor: theme.border, color: theme.text, fontFamily: fonts.medium }]} value={option.label} />{poll.options.length > 2 ? <Pressable accessibilityLabel={`선택지 ${index + 1} 삭제`} onPress={() => setPoll((current) => ({ ...current, options: current.options.filter((item) => item.id !== option.id) }))} style={styles.removeOption}><X color={theme.muted} size={17} /></Pressable> : null}</View>)}</View>
        {poll.options.length < MAX_POLL_OPTIONS ? <Pressable onPress={() => setPoll((current) => ({ ...current, options: [...current.options, { id: newId(), label: '' }] }))} style={[styles.addOption, { backgroundColor: theme.surface, borderColor: theme.border }]}><Plus color={theme.text} size={17} /><Text style={{ color: theme.text, fontFamily: fonts.medium, fontSize: 14 }}>선택지 추가</Text></Pressable> : null}<View style={[styles.pollActions, { borderTopColor: theme.border }]}>{poll.id ? <Pressable onPress={deletePoll} style={styles.deletePoll}><Text style={{ color: '#ef4444', fontFamily: fonts.medium, fontSize: 14 }}>투표 삭제</Text></Pressable> : <View style={styles.deletePoll} />}<Pressable onPress={savePoll} style={[styles.savePoll, { backgroundColor: theme.ink }]}><Text style={{ color: theme.surface, fontFamily: fonts.medium, fontSize: 14 }}>{poll.id ? '수정' : '추가'}</Text></Pressable></View>
      </BottomSheet>
    </View>
  );
}
