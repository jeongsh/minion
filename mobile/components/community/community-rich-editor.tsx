...
export const CommunityRichEditor = forwardRef<CommunityRichEditorHandle, Props>(function CommunityRichEditor({ allowEmbeds, allowMedia, characterCount, characterLimit, maxImages, miniconPacks, onChange, value }, ref) {
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.surface }]}>
      ...
      {uploading ? <View accessibilityLiveRegion="polite" accessibilityRole="progressbar" pointerEvents="none" style={[styles.uploadPlaceholder, { backgroundColor: theme.surface, borderColor: theme.border }]}><ActivityIndicator color={theme.accent} size="large" /><View><Text style={{ color: theme.ink, ...fonts.medium, fontSize: 14 }}>이미지를 올리는 중이에요</Text><Text style={{ color: theme.muted, ...fonts.regular, fontSize: 13, lineHeight: 19.5, marginTop: 2 }}>완료되면 이 위치에 이미지가 표시됩니다.</Text></View></View> : null}
