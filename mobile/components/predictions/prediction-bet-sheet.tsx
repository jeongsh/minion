        ...
        <View style={styles.content}>
          {dialog.existingBet ? (
            <View style={[styles.existing, { backgroundColor: theme.card }]}>
              <Text style={[styles.existingTitle, { color: theme.ink, ...fonts.bold }]}>이미 이 경기에 {dialog.existingBet.stake.toLocaleString('ko-KR')} LP를 사용했습니다.</Text>
              <Text style={[styles.help, { color: theme.muted, ...fonts.regular }]}>팀이나 금액을 바꾸려면 기존 예측을 취소한 뒤 다시 참여해 주세요.</Text>
              <Pressable disabled={pending} onPress={onCancelBet} style={({ pressed }) => [styles.cancelButton, { opacity: pending || pressed ? 0.65 : 1 }]}>
                {pending ? <ActivityIndicator color="#ffffff" /> : <Text style={[styles.primaryText, { ...fonts.black }]}>예측 취소하고 LP 환불</Text>}
              </Pressable>
            </View>
          ) : (
            <>
              <View style={styles.labelRow}>
                <Text style={[styles.label, { color: theme.ink, ...fonts.bold }]}>사용할 LP</Text>
                <Text style={[styles.limit, { color: theme.muted, ...fonts.medium }]}>1회 한도 {maxStake.toLocaleString('ko-KR')} LP</Text>
              </View>
              <View style={[styles.inputWrap, { borderColor: error ? '#ef4444' : theme.border }]}>
                <TextInput
                  ...
                  selectTextOnFocus
                  style={[styles.input, { color: theme.ink, ...fonts.black }]}
                  value={stake}
                />
                <Text style={[styles.unit, { color: theme.muted, ...fonts.black }]}>LP</Text>
              </View>
              <View style={styles.ratios}>
                {[0.25, 0.5, 0.75, 1].map((ratio) => (
                  <Pressable disabled={pending} key={ratio} onPress={() => setRatio(ratio)} style={({ pressed }) => [styles.ratioButton, { backgroundColor: theme.card, opacity: pressed ? 0.7 : 1 }]}>
                    <Text style={[styles.ratioText, { color: theme.text, ...fonts.medium }]}>{ratio === 1 ? '최대' : `${ratio * 100}%`}</Text>
                  </Pressable>
                ))}
              </View>
              <Text style={[styles.help, { color: valid ? theme.muted : '#ef4444', ...fonts.medium }]}>
                {valid
              ...
              <Pressable disabled={!valid || pending} onPress={onSubmit} style={({ pressed }) => [styles.submitButton, { backgroundColor: theme.ink, opacity: !valid || pending ? 0.4 : pressed ? 0.8 : 1 }]}>
                {pending ? <ActivityIndicator color={theme.surface} /> : <Text style={[styles.submitText, { color: theme.surface, ...fonts.black }]}>{amount.toLocaleString('ko-KR')} LP로 확정</Text>}
              </Pressable>
            </>
          )}
          {error ? <Text accessibilityLiveRegion="polite" style={[styles.error, { ...fonts.medium }]}>{error}</Text> : null}
        </View>
      ) : null}
