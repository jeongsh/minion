    ...
    <View style={[styles.choice, reverse && styles.choiceReverse]}>
      <View style={[styles.choiceIdentity, reverse && styles.choiceReverse]}>
        ...
        <View style={[styles.choiceLabel, reverse && styles.choiceReverse]}>
          <Text numberOfLines={1} style={{ color: theme.ink, fontFamily: fonts.black, fontSize: 15, lineHeight: 20 }}>{team?.shortName ?? 'TBD'}</Text>
          <Text style={{ color: theme.muted, fontFamily: fonts.medium, fontSize: 11, lineHeight: 16 }}>{oddsLabel(odds)} 배</Text>
        </View>
      </View>
      <Text style={{ color: theme.ink, fontFamily: fonts.black, fontSize: 17, lineHeight: 20 }}>{percent}<Text style={{ color: theme.muted, fontFamily: fonts.black, fontSize: 12 }}>%</Text></Text>
    </View>
  );
}
...
function PredictionBar({ data }: { data: MobileMatchDetailDto }) {
  ...
  return (
    <View style={[styles.prediction, { backgroundColor: colorScheme === 'dark' ? theme.surfaceMuted : theme.surface, borderColor: theme.border }]}>
      ...
      <View style={styles.predictionVs}><Text style={{ color: theme.muted, fontFamily: fonts.black, fontSize: 13 }}>VS</Text></View>
  ...
  return (
    <View style={styles.watchPointLine}>
      <View style={styles.watchPointLead}>
        ...
        <Text style={[styles.watchPointText, { color: theme.ink, fontFamily: fonts.bold }]}>{lead}</Text>
      </View>
      {rest ? <Text style={[styles.watchPointText, { color: theme.ink, fontFamily: fonts.bold }]}> {rest}</Text> : null}
    </View>
  );
}
...
function BriefingRow({ children, label }: { children: React.ReactNode; label: string }) {
  ...
  return (
    <View style={[styles.briefingRow, { backgroundColor: theme.card }]}>
      <Text style={{ color: theme.muted, fontFamily: fonts.bold, fontSize: 13, lineHeight: 19.5 }}>{label}</Text>
      {children}
    </View>
  );
}
...
function MetricCard({ colorA, colorB, label, valueA, valueB }: { colorA: string; colorB: string; label: string; valueA: string; valueB: string }) {
  ...
  return (
    <View style={[styles.metricCard, { backgroundColor: theme.surface }]}>
      <Text style={{ color: theme.muted, fontFamily: fonts.bold, fontSize: 13, lineHeight: 19.5 }}>{label}</Text>
      <Text numberOfLines={1} style={{ fontFamily: fonts.black, fontSize: 14, lineHeight: 21 }}>
...
function MeetingSide({ match, side }: { match: MobileMatchSummary; side: 'a' | 'b' }) {
  ...
  return (
    <View style={[styles.meetingSide, side === 'a' && styles.meetingSideA]}>
      <Text style={{ color: won ? theme.ink : theme.muted, fontFamily: won ? fonts.black : fonts.bold, fontSize: 16, lineHeight: 20 }}>{score ?? '-'}</Text>
      <Text numberOfLines={1} style={{ color: won ? theme.ink : theme.muted, fontFamily: won ? fonts.black : fonts.bold, fontSize: 14, lineHeight: 20 }}>{team?.shortName ?? 'TBD'}</Text>
...
function MeetingRow({ match }: { match: MobileMatchSummary }) {
  ...
  return (
    <View style={[styles.meetingRow, { backgroundColor: theme.surface }]}>
      <Text style={{ color: theme.muted, fontFamily: fonts.medium, fontSize: 12, lineHeight: 18 }}>{meetingDate(match.startsAt)}</Text>
      <View style={styles.meetingPair}>
        <MeetingSide match={match} side="a" />
        <Text style={{ color: theme.muted, fontFamily: fonts.medium, fontSize: 12 }}>:</Text>
        <MeetingSide match={match} side="b" />
      </View>
      <View style={[styles.winnerPill, { backgroundColor: winnerColor }]}><Text style={{ color: '#ffffff', fontFamily: fonts.medium, fontSize: 12 }}>{winner?.shortName ?? '-'} 승</Text></View>
    </View>
  );
}
...
export function MatchPreviewTab({ data }: { data: MobileMatchDetailDto }) {
  ...
  return (
    <View style={styles.root}>
      ...
      <View>
        <Text style={[styles.briefingHeading, { color: theme.ink, fontFamily: fonts.display }]}>AI 브리핑</Text>
        <BriefingRow label="판세 요약"><Text style={{ color: theme.text, fontFamily: fonts.regular, fontSize: 14, lineHeight: 24 }}>{ai.summary}</Text></BriefingRow>
        <BriefingRow label="눈여겨볼 곳">
          <WatchPoint lead={lead} rest={rest} />
        </BriefingRow>
        {ai.winProbabilityA !== null ? (
          <BriefingRow label="예상 승률">
            <View style={styles.winRow}><View style={styles.winTeam}><TeamLogo plain size={20} team={data.match.teamA} themeAware /><Text style={{ color: theme.ink, fontFamily: fonts.black, fontSize: 14 }}>{ai.winProbabilityA}%</Text></View><View style={styles.winTeam}><Text style={{ color: theme.ink, fontFamily: fonts.black, fontSize: 14 }}>{100 - ai.winProbabilityA}%</Text><TeamLogo plain size={20} team={data.match.teamB} themeAware /></View></View>
        ...
        <BriefingRow label="최근 맞대결">
          {meetings.length === 0 ? <Text style={{ color: theme.muted, fontFamily: fonts.regular, fontSize: 14, lineHeight: 21 }}>현재 수집된 기록 기준 첫 맞대결입니다. 최근 대진 난이도와 경기력으로 비교했습니다.</Text> : <View style={styles.meetingList}>{meetings.map((match) => <MeetingRow key={match.id} match={match} />)}</View>}
        </BriefingRow>
        {ai.sources.length > 0 ? <BriefingRow label="참고한 전망"><View style={styles.sources}>{ai.sources.map((source) => <Pressable key={source.url} onPress={() => Linking.openURL(source.url)} style={styles.sourceLink}><Text numberOfLines={1} style={{ color: theme.text, fontFamily: fonts.bold, fontSize: 14, textDecorationLine: 'underline' }}>{source.title}</Text><ExternalLink color={theme.text} size={12} /></Pressable>)}</View></BriefingRow> : null}
      </View>
    </View>
  );
}
