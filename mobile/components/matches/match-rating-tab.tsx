  ...
  return (
    <View accessibilityLabel={`${player.name} ${player.position} 눌러서 평가하기 ${player.averageRating?.toFixed(1) ?? '-'} ${player.ratingCount}명 참여`} style={[styles.playerRow, { backgroundColor: theme.card, opacity: 0.9 }]}>
      ...
      <View style={styles.playerIdentity}>
        <View style={styles.playerNameRow}><Text numberOfLines={1} style={{ color: theme.ink, ...fonts.bold, fontSize: 15, lineHeight: 20 }}>{player.name}</Text><Text style={{ color: theme.muted, ...fonts.regular, fontSize: 12 }}>{player.position}</Text>{player.isPog ? <View style={styles.pog}><Text style={{ color: '#000000', ...fonts.regular, fontSize: 12 }}>POG</Text></View> : null}</View>
        <Text style={{ color: theme.muted, ...fonts.regular, fontSize: 12, lineHeight: 16 }}>눌러서 평가하기</Text>
      </View>
      <View style={styles.ratingSummary}>
        ...
        <Text style={{ color: theme.muted, ...fonts.regular, fontSize: 12 }}>{player.ratingCount}명 참여</Text>
      </View>
    </View>
  );
}
...
function TeamRatingGroup({ players }: { players: MobileFanRatingPlayer[] }) {
  ...
  return (
    <View style={styles.teamGroup}>
      <View style={styles.teamHeading}><TeamLogo plain size={28} team={team} themeAware /><Text style={{ color: theme.ink, ...fonts.black, fontSize: 16 }}>{team.shortName}</Text></View>
  ...
  return (
    <View style={[styles.comment, { backgroundColor: theme.card }]}>
      <View style={styles.commentHead}>
        ...
        <Text numberOfLines={1} style={{ color: theme.ink, ...fonts.bold, fontSize: 14 }}>{item.authorName}</Text>
        <View style={[styles.playerBadge, { backgroundColor: theme.surface }]}>{item.playerImage?.url ? <Image contentFit="cover" contentPosition="top" source={{ uri: resolveApiAssetUrl(item.playerImage.url) ?? undefined }} style={styles.badgeImage} /> : null}<Text style={{ color: theme.muted, ...fonts.regular, fontSize: 12 }}>{item.playerName}</Text></View>
      ...
      <Text style={{ color: theme.text, ...fonts.regular, fontSize: 14, lineHeight: 24, marginTop: 6 }}>{item.review}</Text>
...
export function MatchRatingTab({ panel }: { panel: MobileFanRatingPanel | null }) {
  ...
  if (!panel) return <View style={[styles.empty, { borderColor: theme.border }]}><Text style={{ color: theme.muted, ...fonts.regular, fontSize: 15 }}>투표할 세트가 없습니다.</Text></View>;
  ...
  return (
    <View style={styles.root}>
      <View style={[styles.loginBanner, { backgroundColor: theme.card }]}><Text style={{ color: theme.muted, flex: 1, ...fonts.regular, fontSize: 14 }}>평점을 남기려면 로그인이 필요합니다.</Text><Pressable onPress={() => showToast('로그인은 인증 화면 구현 단계에서 연결합니다.')} style={[styles.loginButton, { backgroundColor: theme.ink }]}><Text style={{ color: theme.surface, ...fonts.bold, fontSize: 15 }}>로그인</Text></Pressable></View>
      {teams.map((teamId) => <TeamRatingGroup key={teamId} players={panel.players.filter((player) => player.team?.id === teamId)} />)}
      <Text style={{ color: theme.muted, ...fonts.regular, fontSize: 14, lineHeight: 21 }}>{panel.statusNote}</Text>
      <View style={styles.commentsSection}>
        <View style={styles.commentsHead}><Text style={{ color: theme.ink, ...fonts.black, fontSize: 16 }}>평가 코멘트</Text>{panel.comments.length > 0 ? <Pressable onPress={() => setFilterOpen(true)} style={[styles.filter, { backgroundColor: theme.card }]}><Text numberOfLines={1} style={{ color: theme.ink, ...fonts.regular, fontSize: 14 }}>{playerId ? `${filterPlayers.find((item) => item.id === playerId)?.name} (${comments.length})` : `전체 선수 (${panel.comments.length})`}</Text><ChevronDown color={theme.muted} size={16} /></Pressable> : null}</View>
        {comments.length === 0 ? <Text style={{ color: theme.muted, ...fonts.regular, fontSize: 14, marginTop: 8 }}>아직 작성된 평가 코멘트가 없습니다.</Text> : <View style={styles.commentList}>{comments.map((item) => <RatingComment item={item} key={item.id} />)}</View>}
      </View>
      <BottomSheet onClose={() => setFilterOpen(false)} open={filterOpen} title="선수별 코멘트 필터"><View style={styles.filterList}><Pressable onPress={() => { setPlayerId(null); setFilterOpen(false); }} style={styles.filterItem}><Text style={{ color: theme.ink, ...fonts.bold, fontSize: 16 }}>전체 선수 ({panel.comments.length})</Text></Pressable>{filterPlayers.map((player) => <Pressable key={player.id} onPress={() => { setPlayerId(player.id); setFilterOpen(false); }} style={styles.filterItem}><Text style={{ color: theme.ink, ...fonts.bold, fontSize: 16 }}>{player.name} ({panel.comments.filter((item) => item.playerId === player.id).length})</Text></Pressable>)}</View></BottomSheet>
    </View>
  );
}
