  ...
  return (
    <Pressable onPress={() => router.navigate(`/matches/${encodeURIComponent(match.id)}` as never)} style={[styles.matchCard, { backgroundColor: theme.card, width: cardWidth }]}>
      <View style={styles.matchMetaRow}>
        <View style={styles.tournamentMeta}><Image contentFit="contain" source={kespa ? { uri: resolveApiAssetUrl('/logos/tournaments/kespa-cup.webp') ?? undefined } : colorScheme === 'dark' ? LCK_LOGO_DARK : LCK_LOGO} style={styles.tournamentLogo} /><Text numberOfLines={1} style={[styles.matchMeta, styles.flex, { color: theme.muted, fontFamily: fonts.medium }]}>{tournamentName}</Text></View>
        <Text style={[styles.matchMeta, styles.matchDate, { color: theme.muted, fontFamily: fonts.medium }]}>{matchDate(match.startsAt)}</Text>
      </View>
      <View style={styles.matchTeams}>
        <View style={styles.matchSideLeft}><Text numberOfLines={1} style={[styles.matchTeamName, { color: theme.ink, fontFamily: fonts.black }]}>{match.teamA?.shortName ?? 'TBD'}</Text><TeamLogo plain size={28} team={match.teamA} themeAware /></View>
        <View style={styles.timePill}>{live ? <View style={styles.liveDot} /> : null}<Text style={{ color: '#fff', fontFamily: fonts.medium, fontSize: 12, fontVariant: ['tabular-nums'], lineHeight: 18 }}>{centerLabel}</Text></View>
        <View style={styles.matchSideRight}><TeamLogo plain size={28} team={match.teamB} themeAware /><Text numberOfLines={1} style={[styles.matchTeamName, { color: theme.ink, fontFamily: fonts.black }]}>{match.teamB?.shortName ?? 'TBD'}</Text></View>
      </View>
      <View style={styles.oddsArea}><View style={styles.oddsLabels}><Text style={[styles.oddsText, { color: theme.muted, fontFamily: fonts.medium }]}>50%</Text><Text style={[styles.oddsText, { color: theme.muted, fontFamily: fonts.medium }]}>50%</Text></View><View style={[styles.oddsTrack, { backgroundColor: theme.surface }]}><View style={[styles.oddsHalf, { backgroundColor: match.teamA?.primaryColor ?? '#b49b57' }]} /><View style={[styles.oddsHalf, { backgroundColor: match.teamB?.primaryColor ?? '#ef3340' }]} /></View></View>
    </Pressable>
  );
}
...
function SectionHeader({ action, title }: { action?: () => void; title: string }) {
  ...
  return <View style={styles.sectionHeader}><Text style={[styles.sectionTitle, { color: theme.ink, fontFamily: fonts.display }]}>{title}</Text>{action ? <Pressable onPress={action} style={styles.sectionAction}><Text style={[styles.sectionActionText, { color: theme.muted, fontFamily: fonts.bold }]}>전체보기</Text><ChevronRight color={theme.muted} size={16} /></Pressable> : null}</View>;
}
