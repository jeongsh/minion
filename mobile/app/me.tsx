...
export default function MeScreen() {
  ...
  return <View style={[styles.root, { backgroundColor: theme.pageBackground }]}><MinionScreen contentStyle={styles.content} scrollRequest={scrollRequest}>
    {authLoading || (!data && !error) ? <AccountSkeleton /> : error || !data ? <View style={[styles.errorCard, { borderColor: theme.border }]}><Text style={{ color: theme.text, ...fonts.medium, textAlign: 'center' }}>{error}</Text><Pressable onPress={() => void load()} style={[styles.retry, { backgroundColor: theme.accent }]}><Text style={{ color: theme.accentForeground, ...fonts.bold }}>다시 시도</Text></Pressable></View> : <>
      <AccountSummary checkingIn={checkingIn} data={data} onCheckIn={() => void checkIn()} onLogout={() => void signOut()} onOpenActivity={() => router.push(`/community/user/${data.profile.id}` as never)} />
      <Pressable accessibilityRole="button" onPress={() => router.push('/me/minicons' as never)} style={[styles.miniconLink, { backgroundColor: theme.surface, borderColor: theme.border }]}><Sticker color={theme.muted} size={16} /><Text numberOfLines={1} style={[styles.miniconLabel, { color: theme.text, ...fonts.medium }]}>내 미니콘</Text><ChevronRight color={theme.muted} size={15} /></Pressable>
...
function AccountSummary({ checkingIn, data, onCheckIn, onLogout, onOpenActivity }: { checkingIn: boolean; data: MobileMeDto; onCheckIn: () => void; onLogout: () => void; onOpenActivity: () => void }) {
  ...
  return <View style={[styles.summary, { backgroundColor: theme.surface, borderColor: theme.border }]}><View style={styles.summaryRow}><RankAvatar fallback={initials} profileImageUrl={data.profile.profileImage?.url} reserveMedalSpace size="mobile" tier={data.profile.tier} /><View style={styles.summaryCopy}><Text numberOfLines={1} style={[styles.nickname, { color: theme.ink, ...fonts.black }]}>{data.profile.nickname ?? 'MINION 팬'}</Text><Text numberOfLines={1} style={[styles.account, { color: theme.muted, ...fonts.medium }]}>{accountLabel}</Text><View style={styles.rankLine}><Text style={{ color: theme.ink, ...fonts.medium, fontSize: 13, lineHeight: 18 }}>{data.rank.progressLabel}</Text><Text style={{ color: theme.muted, ...fonts.medium, fontSize: 13, lineHeight: 18 }}>{data.profile.lp.toLocaleString('ko-KR')} LP</Text>{data.rank.overallRank ? <Text style={{ color: theme.muted, ...fonts.medium, fontSize: 13, lineHeight: 18 }}>전체 {data.rank.overallRank.toLocaleString('ko-KR')}위</Text> : null}</View></View></View><View style={[styles.progressTrack, { backgroundColor: theme.surfaceMuted }]}><View style={[styles.progressBar, { backgroundColor: theme.accent, width: `${Math.round(data.rank.progressRatio * 100)}%` }]} /></View><Pressable disabled={checkingIn || data.rank.checkedInToday} hitSlop={4} onPress={onCheckIn} style={[styles.checkIn, { backgroundColor: theme.accent, opacity: data.rank.checkedInToday ? 0.7 : 1 }]}>{checkingIn ? <ActivityIndicator color={theme.accentForeground} size="small" /> : <Text style={{ color: theme.accentForeground, ...fonts.medium, fontSize: 13 }}>{data.rank.checkedInToday ? '오늘 출석 완료' : '출석체크 (+100 LP)'}</Text>}</Pressable><View style={styles.summaryActions}><Pressable hitSlop={4} onPress={onOpenActivity} style={[styles.summaryAction, { backgroundColor: theme.surfaceMuted }]}><FileText color={theme.text} size={14} /><Text style={{ color: theme.text, ...fonts.medium, fontSize: 13 }}>내 활동</Text></Pressable><Pressable hitSlop={4} onPress={onLogout} style={[styles.summaryAction, { backgroundColor: theme.surfaceMuted }]}><LogOut color="#dc2626" size={14} /><Text style={{ color: '#dc2626', ...fonts.medium, fontSize: 13 }}>로그아웃</Text></Pressable></View></View>;
}
...
const styles = StyleSheet.create({ root: { flex: 1 }, content: { gap: 10 }, summary: { borderRadius: 12, borderWidth: 1, overflow: 'visible', padding: 16 }, summaryRow: { alignItems: 'center', flexDirection: 'row', gap: 10, overflow: 'visible' }, summaryCopy: { flex: 1, minWidth: 0 }, nickname: { fontSize: 15, letterSpacing: -0.3, lineHeight: 22 }, account: { fontSize: 13, lineHeight: 18 }, rankLine: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 2 }, progressTrack: { borderRadius: 999, height: 4, marginTop: 8, overflow: 'hidden' }, progressBar: { borderRadius: 999, height: 4 }, checkIn: { alignItems: 'center', borderRadius: 8, height: 36, justifyContent: 'center', marginBottom: 4, marginTop: 8 }, summaryActions: { flexDirection: 'row', gap: 8, marginTop: 6 }, summaryAction: { alignItems: 'center', borderRadius: 8, flex: 1, flexDirection: 'row', gap: 5, height: 36, justifyContent: 'center' }, miniconLink: { alignItems: 'center', borderRadius: 12, borderWidth: 1, flexDirection: 'row', gap: 8, height: 44, paddingHorizontal: 12 }, miniconLabel: { flex: 1, fontSize: 14, lineHeight: 20 }, menuGrid: { borderRadius: 12, borderWidth: 1, flexDirection: 'row', gap: 6, padding: 8 }, menuItem: { alignItems: 'center', borderRadius: 8, flex: 1, flexDirection: 'row', gap: 3, height: 36, justifyContent: 'center', minWidth: 0, paddingHorizontal: 1 }, errorCard: { alignItems: 'center', borderRadius: 12, borderWidth: 1, gap: 16, padding: 24 }, retry: { borderRadius: 8, paddingHorizontal: 18, paddingVertical: 12 }, skeletonCircle: { borderRadius: 28, height: 56, width: 56 }, skeletonLine: { borderRadius: 4, height: 14 } });
