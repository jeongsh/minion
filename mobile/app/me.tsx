...
const menuItems = [
  { key: 'profile' as const, label: '프로필', icon: UserRound }, { key: 'minicons' as const, label: '미니콘', icon: Sticker }, { key: 'notifications' as const, label: '알림', icon: Bell },
...
export default function MeScreen() {
  ...
  return <View style={[styles.root, { backgroundColor: theme.pageBackground }]}><MinionScreen contentStyle={styles.content} scrollRequest={scrollRequest}>
    {authLoading || (!data && !error) ? <AccountSkeleton /> : error || !data ? <View style={[styles.errorCard, { borderColor: theme.border }]}><Text style={{ color: theme.text, ...fonts.medium, textAlign: 'center' }}>{error}</Text><Pressable onPress={() => void load()} style={[styles.retry, { backgroundColor: theme.accent }]}><Text style={{ color: theme.accentForeground, ...fonts.bold }}>다시 시도</Text></Pressable></View> : <>
      ...
      <View accessibilityRole="tablist" style={[styles.menuGrid, { backgroundColor: theme.surface, borderColor: theme.border }]}>{menuItems.map(({ icon: Icon, key, label }) => <Pressable accessibilityRole="tab" hitSlop={4} key={key} onPress={() => key === 'minicons' ? router.push('/me/minicons' as never) : navigateSection(key)} style={[styles.menuItem, { backgroundColor: theme.surfaceMuted }]}><Icon color={theme.muted} size={13} /><Text numberOfLines={1} style={{ color: theme.text, ...fonts.medium, fontSize: 13, lineHeight: 18 }}>{label}</Text></Pressable>)}</View>
