...
| --- | --- | --- | --- |
| 인앱 알림 | `인앱 알림` | 켜짐 | 해당 없음(웹 전용 — 아래 3개 알림을 브라우저/앱이 켜져있을 때 토스트로 보여주는 별도 채널, 로컬스토리지 기반) |
| 경기 시작 | `경기 시작` | 켜짐(옵트아웃) | ✅ 구현됨(모바일 푸시 + 웹 토스트) |
| 경기 주요 이벤트 | `경기 주요 이벤트` | 꺼짐(옵트인) | ✅ 구현됨(모바일 푸시 + 웹 토스트) |
| 세트 평가 오픈 | `세트 평가 오픈` | 켜짐 | ✅ 구현됨(모바일 푸시 + 웹 토스트) |
...
- 이 문서는 모바일 푸시 기준으로 쓰여있지만, 같은 3개 이벤트는 웹에서 `components/match-activity/use-match-activity.ts`의 `publishNotification`을 통해 토스트(+ 알림함)로도 나간다. 발송 트리거·중복 방지(DB 폴링 쪽)는 서로 다른 코드 경로라 독립적으로 관리한다 — 2.1 참고.
