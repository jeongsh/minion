...
- [x] 팀 상세 slug 변경 시 기존 URL redirect 정책 결정 (slug 변경 최소화, 변경 시 이전 URL은 404 처리)
- [x] `/admin/players`에서 실제 선수 생성/수정 폼 구현
