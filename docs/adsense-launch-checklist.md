...
> 2026-09-20: 광고 영역을 임시로 모두 중단한다. `NEXT_PUBLIC_ADSENSE_ENABLED` 미설정 또는 `false`이면 아래 슬롯 설정과 관계없이 수동 광고·임시 자리·자동 광고 스크립트가 꺼진다. 사이트 확인용 메타 태그와 `ads.txt`는 유지한다. 재개는 이 변수를 `true`로 설정한 뒤 Git 기반 재배포하며, 상세 기준은 `docs/ads-content-scope.md`를 따른다.
